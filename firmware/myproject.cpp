...
void myproject(
    hls::stream<input29_t> em_barrel[4], hls::stream<input13_t> scalars[1], hls::stream<input_t> tracks[6],
    ...
    hls::stream<layer117_t> layer117_out[1];
    #pragma HLS STREAM variable=layer117_out depth=57
    nnet::pointwise_conv_1d_cl_switch<layer103_t, layer117_t, config117>(layer103_cpy1, layer117_out, w117, b117); // conv1d_1

    hls::stream<layer4_t> layer4_out[1];
    #pragma HLS STREAM variable=layer4_out depth=57
    nnet::leaky_relu_switch<layer117_t, layer4_t, LeakyReLU_config4>(layer117_out, 0.30000001192092896, layer4_out); // leaky_re_lu_1

    hls::stream<layer5_t> layer5_out[1];
    #pragma HLS STREAM variable=layer5_out depth=57
    nnet::mask_track_switch<layer103_t, layer5_t, config5>(layer103_cpy2, layer5_out); // mask_track

    hls::stream<layer118_t> layer118_out[1];
    #pragma HLS STREAM variable=layer118_out depth=57
    nnet::pointwise_conv_1d_cl_switch<layer4_t, layer118_t, config118>(layer4_out, layer118_out, w118, b118); // conv1d_2

    hls::stream<layer119_t> layer119_out[1];
    #pragma HLS STREAM variable=layer119_out depth=57
    nnet::pointwise_conv_1d_cl_switch<layer5_t, layer119_t, config119>(layer5_out, layer119_out, w119, b119); // conv1d

    hls::stream<layer10_t> layer10_out[1];
    #pragma HLS STREAM variable=layer10_out depth=57
    nnet::leaky_relu_switch<layer118_t, layer10_t, LeakyReLU_config10>(layer118_out, 0.30000001192092896, layer10_out); // leaky_re_lu_2

    hls::stream<layer11_t> layer11_out[1];
    #pragma HLS STREAM variable=layer11_out depth=57
    nnet::multiply_switch<layer119_t, layer10_t, layer11_t, config11>(layer119_out, layer10_out, layer11_out); // multiply

    hls::stream<layer12_t> layer12_out[1];
    #pragma HLS STREAM variable=layer12_out depth=1
    nnet::sum1d_switch<layer11_t, layer12_t, config12>(layer11_out, layer12_out); // sum1d
    ...
    nnet::leaky_relu_switch<layer14_t, layer18_t, LeakyReLU_config18>(layer14_out, 0.30000001192092896, layer18_out); // leaky_re_lu_3

    hls::stream<layer19_t> layer19_out[1];
    #pragma HLS STREAM variable=layer19_out depth=1
    nnet::normalize_switch<layer16_t, layer19_t, config19>(layer16_out, layer19_out, s19, b19); // batch_normalization
    ...
    nnet::leaky_relu_switch<layer19_t, layer22_t, LeakyReLU_config22>(layer19_out, 0.30000001192092896, layer22_out); // leaky_re_lu

    hls::stream<layer23_t> layer23_out[1];
    #pragma HLS STREAM variable=layer23_out depth=1
    nnet::leaky_relu_switch<layer20_t, layer23_t, LeakyReLU_config23>(layer20_out, 0.30000001192092896, layer23_out); // leaky_re_lu_4

    hls::stream<layer24_t> layer24_out[1];
    #pragma HLS STREAM variable=layer24_out depth=1
    nnet::concatenate1d_switch<layer22_t, layer23_t, layer24_t, config24>(layer22_out, layer23_out, layer24_out); // concatenate
    ...
    nnet::normalize_switch<layer25_t, layer27_t, config27>(layer25_out, layer27_out, s27, b27); // batch_normalization_2

    hls::stream<layer28_t> layer28_out[1];
    #pragma HLS STREAM variable=layer28_out depth=1
    nnet::leaky_relu_switch<layer27_t, layer28_t, LeakyReLU_config28>(layer27_out, 0.30000001192092896, layer28_out); // leaky_re_lu_5
    ...
    hls::stream<layer32_t> layer32_out[4];
    #pragma HLS STREAM variable=layer32_out depth=3080
    nnet::resize_nearest_switch<input29_t, config32>(em_barrel, layer32_out); // up_sampling2d

    hls::stream<layer33_t> layer33_out[1];
    #pragma HLS STREAM variable=layer33_out depth=1
    nnet::normalize_switch<layer30_t, layer33_t, config33>(layer30_out, layer33_out, s33, b33); // batch_normalization_3

    hls::stream<layer34_t> layer34_out[4];
    #pragma HLS STREAM variable=layer34_out depth=3080
    nnet::normalize_switch<layer32_t, layer34_t, config34>(layer32_out, layer34_out, s34, b34); // batch_normalization_1

    hls::stream<layer108_t> layer108_out[4];
    #pragma HLS STREAM variable=layer108_out depth=3540
    nnet::zeropad2d_cl_switch<layer34_t, layer108_t, config108>(layer34_out, layer108_out); // zp2d_conv2d

    hls::stream<layer35_t> layer35_out[1];
    #pragma HLS STREAM variable=layer35_out depth=1
    nnet::leaky_relu_switch<layer33_t, layer35_t, LeakyReLU_config35>(layer33_out, 0.30000001192092896, layer35_out); // leaky_re_lu_6

    hls::stream<layer36_t> layer36_out[16];
    #pragma HLS STREAM variable=layer36_out depth=3080
    nnet::conv_2d_cl_switch<layer108_t, layer36_t, config36>(layer108_out, layer36_out, w36, b36); // conv2d
    ...
    hls::stream<layer104_t> layer104_cpy1[1];
    #pragma HLS STREAM variable=layer104_cpy1 depth=1
    hls::stream<layer104_t> layer104_cpy2[1];
    #pragma HLS STREAM variable=layer104_cpy2 depth=1
    nnet::clone_stream_switch<layer38_t, layer104_t, config104>(layer38_out, layer104_cpy1, layer104_cpy2); // clone_dense_5

    hls::stream<layer40_t> layer40_out[16];
    #pragma HLS STREAM variable=layer40_out depth=3080
    nnet::normalize_switch<layer36_t, layer40_t, config40>(layer36_out, layer40_out, s40, b40); // batch_normalization_4

    hls::stream<layer41_t> layer41_out[32];
    #pragma HLS STREAM variable=layer41_out depth=1
    nnet::slice_tensor1d_switch<layer104_t, layer41_t, config41>(layer104_cpy1, layer41_out); // slice_tensor1d

    hls::stream<layer42_t> layer42_out[16];
    #pragma HLS STREAM variable=layer42_out depth=3080
    nnet::film_switch<layer40_t, layer41_t, layer42_t, config42>(layer40_out, layer41_out, layer42_out); // fi_lm

    hls::stream<layer43_t> layer43_out[16];
    #pragma HLS STREAM variable=layer43_out depth=3080
    nnet::leaky_relu_switch<layer42_t, layer43_t, LeakyReLU_config43>(layer42_out, 0.30000001192092896, layer43_out); // leaky_re_lu_7

    hls::stream<layer44_t> layer44_out[16];
    #pragma HLS STREAM variable=layer44_out depth=756
    nnet::pooling2d_cl_switch<layer43_t, layer44_t, config44>(layer43_out, layer44_out); // max_pooling2d

    hls::stream<layer109_t> layer109_out[16];
    #pragma HLS STREAM variable=layer109_out depth=870
    nnet::zeropad2d_cl_switch<layer44_t, layer109_t, config109>(layer44_out, layer109_out); // zp2d_conv2d_1

    hls::stream<layer45_t> layer45_out[32];
    #pragma HLS STREAM variable=layer45_out depth=756
    nnet::conv_2d_cl_switch<layer109_t, layer45_t, config45>(layer109_out, layer45_out, w45, b45); // conv2d_1

    hls::stream<layer47_t> layer47_out[1];
    #pragma HLS STREAM variable=layer47_out depth=1
    nnet::slice_tensor1d_switch<layer104_t, layer47_t, config47>(layer104_cpy2, layer47_out); // slice_tensor1d_1

    hls::stream<layer105_t> layer105_cpy1[1];
    #pragma HLS STREAM variable=layer105_cpy1 depth=1
    hls::stream<layer105_t> layer105_cpy2[1];
    #pragma HLS STREAM variable=layer105_cpy2 depth=1
    nnet::clone_stream_switch<layer47_t, layer105_t, config105>(layer47_out, layer105_cpy1, layer105_cpy2); // clone_slice_tensor1d_1

    hls::stream<layer48_t> layer48_out[32];
    #pragma HLS STREAM variable=layer48_out depth=756
    nnet::normalize_switch<layer45_t, layer48_t, config48>(layer45_out, layer48_out, s48, b48); // batch_normalization_5

    hls::stream<layer49_t> layer49_out[1];
    #pragma HLS STREAM variable=layer49_out depth=1
    nnet::slice_tensor1d_switch<layer105_t, layer49_t, config49>(layer105_cpy1, layer49_out); // slice_tensor1d_2

    hls::stream<layer50_t> layer50_out[32];
    #pragma HLS STREAM variable=layer50_out depth=756
    nnet::film_switch<layer48_t, layer49_t, layer50_t, config50>(layer48_out, layer49_out, layer50_out); // fi_lm_1

    hls::stream<layer51_t> layer51_out[32];
    #pragma HLS STREAM variable=layer51_out depth=756
    nnet::leaky_relu_switch<layer50_t, layer51_t, LeakyReLU_config51>(layer50_out, 0.30000001192092896, layer51_out); // leaky_re_lu_8

    hls::stream<layer110_t> layer110_out[32];
    #pragma HLS STREAM variable=layer110_out depth=870
    nnet::zeropad2d_cl_switch<layer51_t, layer110_t, config110>(layer51_out, layer110_out); // zp2d_conv2d_2

    hls::stream<layer52_t> layer52_out[32];
    #pragma HLS STREAM variable=layer52_out depth=756
    nnet::conv_2d_cl_switch<layer110_t, layer52_t, config52>(layer110_out, layer52_out, w52, b52); // conv2d_2

    hls::stream<layer54_t> layer54_out[32];
    #pragma HLS STREAM variable=layer54_out depth=756
    nnet::normalize_switch<layer52_t, layer54_t, config54>(layer52_out, layer54_out, s54, b54); // batch_normalization_6

    hls::stream<layer55_t> layer55_out[32];
    #pragma HLS STREAM variable=layer55_out depth=756
    nnet::leaky_relu_switch<layer54_t, layer55_t, LeakyReLU_config55>(layer54_out, 0.30000001192092896, layer55_out); // leaky_re_lu_9

    hls::stream<layer56_t> layer56_out[32];
    #pragma HLS STREAM variable=layer56_out depth=182
    nnet::pooling2d_cl_switch<layer55_t, layer56_t, config56>(layer55_out, layer56_out); // max_pooling2d_1

    hls::stream<layer111_t> layer111_out[32];
    #pragma HLS STREAM variable=layer111_out depth=240
    nnet::zeropad2d_cl_switch<layer56_t, layer111_t, config111>(layer56_out, layer111_out); // zp2d_conv2d_3

    hls::stream<layer57_t> layer57_out[1];
    #pragma HLS STREAM variable=layer57_out depth=182
    nnet::conv_2d_cl_switch<layer111_t, layer57_t, config57>(layer111_out, layer57_out, w57, b57); // conv2d_3

    hls::stream<layer59_t> layer59_out[1];
    #pragma HLS STREAM variable=layer59_out depth=1
    nnet::slice_tensor1d_switch<layer105_t, layer59_t, config59>(layer105_cpy2, layer59_out); // slice_tensor1d_3

    hls::stream<layer106_t> layer106_cpy1[1];
    #pragma HLS STREAM variable=layer106_cpy1 depth=1
    hls::stream<layer106_t> layer106_cpy2[1];
    #pragma HLS STREAM variable=layer106_cpy2 depth=1
    nnet::clone_stream_switch<layer59_t, layer106_t, config106>(layer59_out, layer106_cpy1, layer106_cpy2); // clone_slice_tensor1d_3

    hls::stream<layer60_t> layer60_out[1];
    #pragma HLS STREAM variable=layer60_out depth=182
    nnet::normalize_switch<layer57_t, layer60_t, config60>(layer57_out, layer60_out, s60, b60); // batch_normalization_7

    hls::stream<layer61_t> layer61_out[1];
    #pragma HLS STREAM variable=layer61_out depth=1
    nnet::slice_tensor1d_switch<layer106_t, layer61_t, config61>(layer106_cpy1, layer61_out); // slice_tensor1d_4

    hls::stream<layer62_t> layer62_out[1];
    #pragma HLS STREAM variable=layer62_out depth=182
    nnet::film_switch<layer60_t, layer61_t, layer62_t, config62>(layer60_out, layer61_out, layer62_out); // fi_lm_2

    hls::stream<layer63_t> layer63_out[1];
    #pragma HLS STREAM variable=layer63_out depth=182
    nnet::leaky_relu_switch<layer62_t, layer63_t, LeakyReLU_config63>(layer62_out, 0.30000001192092896, layer63_out); // leaky_re_lu_10

    hls::stream<layer112_t> layer112_out[1];
    #pragma HLS STREAM variable=layer112_out depth=240
    nnet::zeropad2d_cl_switch<layer63_t, layer112_t, config112>(layer63_out, layer112_out); // zp2d_conv2d_4

    hls::stream<layer64_t> layer64_out[1];
    #pragma HLS STREAM variable=layer64_out depth=182
    nnet::conv_2d_cl_switch<layer112_t, layer64_t, config64>(layer112_out, layer64_out, w64, b64); // conv2d_4

    hls::stream<layer66_t> layer66_out[1];
    #pragma HLS STREAM variable=layer66_out depth=182
    nnet::normalize_switch<layer64_t, layer66_t, config66>(layer64_out, layer66_out, s66, b66); // batch_normalization_8

    hls::stream<layer67_t> layer67_out[1];
    #pragma HLS STREAM variable=layer67_out depth=182
    nnet::leaky_relu_switch<layer66_t, layer67_t, LeakyReLU_config67>(layer66_out, 0.30000001192092896, layer67_out); // leaky_re_lu_11

    hls::stream<layer68_t> layer68_out[1];
    #pragma HLS STREAM variable=layer68_out depth=42
    nnet::pooling2d_cl_switch<layer67_t, layer68_t, config68>(layer67_out, layer68_out); // max_pooling2d_2

    hls::stream<layer113_t> layer113_out[1];
    #pragma HLS STREAM variable=layer113_out depth=72
    nnet::zeropad2d_cl_switch<layer68_t, layer113_t, config113>(layer68_out, layer113_out); // zp2d_conv2d_5

    hls::stream<layer69_t> layer69_out[1];
    #pragma HLS STREAM variable=layer69_out depth=42
    nnet::conv_2d_cl_switch<layer113_t, layer69_t, config69>(layer113_out, layer69_out, w69, b69); // conv2d_5

    hls::stream<layer71_t> layer71_out[1];
    #pragma HLS STREAM variable=layer71_out depth=1
    nnet::slice_tensor1d_switch<layer106_t, layer71_t, config71>(layer106_cpy2, layer71_out); // slice_tensor1d_5

    hls::stream<layer107_t> layer107_cpy1[1];
    #pragma HLS STREAM variable=layer107_cpy1 depth=1
    hls::stream<layer107_t> layer107_cpy2[1];
    #pragma HLS STREAM variable=layer107_cpy2 depth=1
    nnet::clone_stream_switch<layer71_t, layer107_t, config107>(layer71_out, layer107_cpy1, layer107_cpy2); // clone_slice_tensor1d_5

    hls::stream<layer72_t> layer72_out[1];
    #pragma HLS STREAM variable=layer72_out depth=42
    nnet::normalize_switch<layer69_t, layer72_t, config72>(layer69_out, layer72_out, s72, b72); // batch_normalization_9

    hls::stream<layer73_t> layer73_out[1];
    #pragma HLS STREAM variable=layer73_out depth=1
    nnet::slice_tensor1d_switch<layer107_t, layer73_t, config73>(layer107_cpy1, layer73_out); // slice_tensor1d_6

    hls::stream<layer74_t> layer74_out[1];
    #pragma HLS STREAM variable=layer74_out depth=42
    nnet::film_switch<layer72_t, layer73_t, layer74_t, config74>(layer72_out, layer73_out, layer74_out); // fi_lm_3

    hls::stream<layer75_t> layer75_out[1];
    #pragma HLS STREAM variable=layer75_out depth=42
    nnet::leaky_relu_switch<layer74_t, layer75_t, LeakyReLU_config75>(layer74_out, 0.30000001192092896, layer75_out); // leaky_re_lu_12

    hls::stream<layer114_t> layer114_out[1];
    #pragma HLS STREAM variable=layer114_out depth=72
    nnet::zeropad2d_cl_switch<layer75_t, layer114_t, config114>(layer75_out, layer114_out); // zp2d_conv2d_6

    hls::stream<layer76_t> layer76_out[1];
    #pragma HLS STREAM variable=layer76_out depth=42
    nnet::conv_2d_cl_switch<layer114_t, layer76_t, config76>(layer114_out, layer76_out, w76, b76); // conv2d_6

    hls::stream<layer78_t> layer78_out[1];
    #pragma HLS STREAM variable=layer78_out depth=42
    nnet::normalize_switch<layer76_t, layer78_t, config78>(layer76_out, layer78_out, s78, b78); // batch_normalization_10

    hls::stream<layer79_t> layer79_out[1];
    #pragma HLS STREAM variable=layer79_out depth=42
    nnet::leaky_relu_switch<layer78_t, layer79_t, LeakyReLU_config79>(layer78_out, 0.30000001192092896, layer79_out); // leaky_re_lu_13

    hls::stream<layer80_t> layer80_out[1];
    #pragma HLS STREAM variable=layer80_out depth=9
    nnet::pooling2d_cl_switch<layer79_t, layer80_t, config80>(layer79_out, layer80_out); // max_pooling2d_3

    hls::stream<layer115_t> layer115_out[1];
    #pragma HLS STREAM variable=layer115_out depth=25
    nnet::zeropad2d_cl_switch<layer80_t, layer115_t, config115>(layer80_out, layer115_out); // zp2d_conv2d_7

    hls::stream<layer81_t> layer81_out[1];
    #pragma HLS STREAM variable=layer81_out depth=9
    nnet::conv_2d_cl_switch<layer115_t, layer81_t, config81>(layer115_out, layer81_out, w81, b81); // conv2d_7

    hls::stream<layer83_t> layer83_out[1];
    #pragma HLS STREAM variable=layer83_out depth=9
    nnet::normalize_switch<layer81_t, layer83_t, config83>(layer81_out, layer83_out, s83, b83); // batch_normalization_11
    ...
    nnet::film_switch<layer83_t, layer84_t, layer85_t, config85>(layer83_out, layer84_out, layer85_out); // fi_lm_4

    hls::stream<layer86_t> layer86_out[1];
    #pragma HLS STREAM variable=layer86_out depth=9
    nnet::leaky_relu_switch<layer85_t, layer86_t, LeakyReLU_config86>(layer85_out, 0.30000001192092896, layer86_out); // leaky_re_lu_14

    hls::stream<layer116_t> layer116_out[1];
    #pragma HLS STREAM variable=layer116_out depth=25
    nnet::zeropad2d_cl_switch<layer86_t, layer116_t, config116>(layer86_out, layer116_out); // zp2d_conv2d_8

    hls::stream<layer87_t> layer87_out[1];
    #pragma HLS STREAM variable=layer87_out depth=9
    nnet::conv_2d_cl_switch<layer116_t, layer87_t, config87>(layer116_out, layer87_out, w87, b87); // conv2d_8

    hls::stream<layer89_t> layer89_out[1];
    #pragma HLS STREAM variable=layer89_out depth=9
    nnet::normalize_switch<layer87_t, layer89_t, config89>(layer87_out, layer89_out, s89, b89); // batch_normalization_12

    hls::stream<layer90_t> layer90_out[1];
    #pragma HLS STREAM variable=layer90_out depth=9
    nnet::leaky_relu_switch<layer89_t, layer90_t, LeakyReLU_config90>(layer89_out, 0.30000001192092896, layer90_out); // leaky_re_lu_15
    ...
    nnet::normalize_switch<layer92_t, layer94_t, config94>(layer92_out, layer94_out, s94, b94); // batch_normalization_13

    hls::stream<layer95_t> layer95_out[1];
    #pragma HLS STREAM variable=layer95_out depth=1
    nnet::leaky_relu_switch<layer94_t, layer95_t, LeakyReLU_config95>(layer94_out, 0.30000001192092896, layer95_out); // leaky_re_lu_16
    ...
    nnet::normalize_switch<layer96_t, layer98_t, config98>(layer96_out, layer98_out, s98, b98); // batch_normalization_14

    hls::stream<layer99_t> layer99_out[1];
    #pragma HLS STREAM variable=layer99_out depth=1
    nnet::leaky_relu_switch<layer98_t, layer99_t, LeakyReLU_config99>(layer98_out, 0.30000001192092896, layer99_out); // leaky_re_lu_17
    ...
    nnet::relu_switch<layer100_t, result_t, relu_config102>(layer100_out, layer102_out); // activation
